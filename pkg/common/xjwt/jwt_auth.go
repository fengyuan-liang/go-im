...
func CreateToken(uid int64, platform constant.PlatformType, access bool, expiresIn int) (t *JwtToken, err error) {
	...
	claims[constant.USER_UID] = utils.ParseString(uid)
	...
	if access == true {
		...
		// TODO: 开发调试用!!!
		key := constant.RK_SYNC_USER_ACCESS_TOKEN + utils.GetHashTagKey(uid) + ":" + utils.ParseString(platform)
		terr := xredis.Set(key, tokenStr, constant.CONST_DURATION_JWT_ACCESS_TOKEN_EXPIRE_IN_SECOND)
		if terr != nil {
			log.Println(key, terr.Error())
		}
	}

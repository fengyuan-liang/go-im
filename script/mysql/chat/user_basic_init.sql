...
create table if not exists `user_basic` (
    ...
    `identity` int(8)  default '-1' comment '身份信息',
    `client_ip` varchar(50) default '' comment '客户端ip地址',
    `login_time` date default null comment '最后登陆时间',
    `heart_beat_time` date default null comment '最后心跳时间',
    `logout_time` date default null comment '最后登陆时间',
